...
class M_duk extends CI_Model
{
    ...
    public function updateDataDuk()
    {
        $data = [
            ...
            'masa_kerja_golongan' => $this->input->post('mkgt', true),
    ...
    {
        $query = $this->db->query('SELECT gaji_pokok FROM gaji WHERE golongan = (SELECT golongan FROM duk WHERE nip = ' . $nip . ') AND masa_kerja = ((SELECT masa_kerja_golongan FROM duk WHERE nip = ' . $nip . ' )+2)');
        return $query->row_array();
    }
    ...
    {
        $data = [
            'masa_kerja_golongan' => $this->input->post('mkgt2', true),
